...
def multifold(
    ...
    Args:
        ...
        noise_variance: variance of the observational noise.
            Set noise_variance = 0 for noiseless observations
    ...
    This function just calls 'multifold_cholesky()' with the appropriate
    Cholesky factor. It is based on the formulation derived in:

    [D. Ginsbourger and C. Schaerer (2021). Fast calculation of Gaussian
        Process multiple-fold crossvalidation residuals and their covariances.
        arXiv:2101.03108]
    """
...
def multifold_cholesky(
    ...
    Args:
        L (2d array): lower triangular Cholesky factor of covariance matrix
            (L L.T = covariance matrix)
        Y_train (array): training observations
        folds (list of lists): The index subsets
    ...
    Note:
    * The matrix K = L L.T is the covariance matrix of the predicted
    ...
    * For observations including Gaussian noise with fixed variance (v), the
    ...
    # @TODO: I feel we could a bit better harmonize the code in
    #  multifold_cholesky() with the code in loo_cholesky(), so that the
    #  comments, variable names and the flow of logic is a bit more
    #  similar / comparable.
...
def loo(
    ...
    (noise_variance = 0) or fixed variance iid Gaussian noise.
    (residual = observed - predicted)
    This function just calls 'loo_cholesky()' with the appropriate Cholesky
    factor.
...
def loo_cholesky(
    ...
    Note:
    * The matrix K = L L.T is the covariance matrix of the predicted observations
        Y_train
    * For observations including Gaussian noise with fixed variance (v), the
        matrix K is K = (K + v*I) where K[i, j] is the prior covariance of the
        latent GP between the i-th an j-th training location
...
def check_folds_indices(
    ...
    Raises:
        AssertionError: if not 'folds' represents the range [0:n_max-1] of n_max
            indices split into non overlapping subsets
    """
...
def check_lower_triangular(
    ...
    assert checks.is_numeric_np_array(
        arr
    ), f"{argname} must be a numpy array with numeric elements"
...
def check_numeric_array(
    ...
    assert (
        len(arr.shape) == dim
    ), f"{argname} must be a {dim} dimensional array"
