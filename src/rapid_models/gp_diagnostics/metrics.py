...
def evaluate_GP(K, Y_train, folds = None, noise_variance = 0, check_args = True):
    ...
    if check_args: 
        ...
        assert K.shape[0] == Y_train.shape[0] and K.shape[1] == Y_train.shape[0], 'The size of K {} is not compatible with Y_train {}'.format(K.shape, Y_train.shape) # Check that K has correct size
        assert noise_variance >= 0, 'noise_variance must be non-negative' # Check that the noise variance is non-negative
        
        if folds is not None: check_folds_indices(folds, Y_train.shape[0]) # Check that the list of index subsets (list of lists) is valid
...
def evaluate_GP_cholesky(L, Y_train, folds = None, check_args = True):
    ...
    mean, cov, residuals_transformed = multifold_cholesky(L, Y_train, folds, False) if folds is not None else loo_cholesky(L, Y_train, False)
