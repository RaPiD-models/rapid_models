...
# ELD TODO: Is it ok to import all from pyDOE to use this doe as a wrapper?
import numpy as np
...
def fullfact_with_bounds(LBs, UBs, N_xi):
    ...
    inputspace between lowerbound and upperbound in a structured grid-like)
    way with n_x_i points in the i'th-dimension.

    Args:
        LBs (list-like, 1D): lower bounds of the input space. len(LBs) must
          equal len(UBs)
        UBs (list-like, 1D): upper bounds of the input space. len(UBs) must
          equal len(LBs)
        N_xi (list-like, 1D): number of equidistant samples for each
          xi-dimension
    ...
    if np.any(N_xi < 1):
        raise ValueError(
            "All elements of N_xi must be greater than or equal to 1"
        )
    if np.any(LBs > UBs):
        raise ValueError(
...
def lhs_with_bounds(nDim, nSamples, LBs, UBs, random_state=None):
    ...
    between lowerbound and upperbound with n points using a Latin-hypercube
    design.
    ...
    Returns:
        lhs (ndarray): Array of sample points with shape (nSamples, nDim)
            [[x_0,...,x_i],...,[x_0_n,...,x_i_n]]
    """
    ...
    if not LBs.shape == UBs.shape == (nDim,):
        raise ValueError(
            "Shape of LBs, UBs, must be equal and match the (nDim,)"
        )
...
def in_hull(p, hull):
    ...
    if not len(p.shape) == 2:
        raise ValueError(
            "p must be a NxK 2D array of 'N'points in 'K' dimensions. \
                p.shape:  {}".format(
                p
            )
        )
    ...
    if not isinstance(hull, Delaunay):
        ...
        if not len(hull.shape) == 2:
            raise ValueError(
                "hull must be a scipy.spatial.Delaunay object or a MxK 2D array \
                    of 'M'points in 'K' dimensions for which Delaunay triangulation \
                    will be computed"
            )
        if not p.shape[1] == hull.shape[1]:
            raise ValueError(
                "Size of second dimension of p and hull must be the same \
                    (i.e. p and hull must have the same 'K' dimensions."
            )
...
def kmeans_sample(points, N, values=None, mode="center", random_state=42):
    ...
    if not isinstance(mode, str):
        raise TypeError(
            "mode must be one of the following strings: 'center', \
                'center_closest_point', 'min', or 'max'"
        )
    if mode.lower() not in ["center", "center_closest_point", "min", "max"]:
        raise ValueError(
            "mode must be one of the following strings: 'center', \
                'center_closest_point', 'min', or 'max'"
        )
    if mode.lower() in ["min", "max"]:
        if values is None:
            raise TypeError(
                "values=None. If mode is 'min' or 'max', values corresponding to \
                    the points need to be given."
            )
        if not np.array(values).shape[0] == np.array(points).shape[0]:
            raise ValueError(
                "Length of 1D array 'Values' must match 2nd dimension of 2D \
                    array 'points'."
            )
        ...
        if values is None:
            ...
            for q, c in enumerate(cntr):
                ...
                if ret_ixs is None:
                    ...
                    ret_ixs = np.append(
                        ret_ixs, np.where(f_lab & (values == val))[0]
                    )
            return points[ret_ixs], values[ret_ixs]
